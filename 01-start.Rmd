...
使用`dplyr`版本的`filter()`。虽然这是个好习惯，但是很少人这么做（除非你是开发者）。Python里每使用一次NumPy里的函数都要加上`np_`的前缀，虽然严谨，但是麻烦。R的一大便利之处就是使用包里的内容时，不强制要求指定包的名称。如果函数/对象名称有重叠，以包的加载顺序决定优先度；最近（即最后）被加载的包里的函数/对象胜出，而其余的要通过`packageName::object`的形式调取。
