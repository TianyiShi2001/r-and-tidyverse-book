...
本节简要解释了R中的基础统计学函数，t分布，t检验和$\chi^2$检验。统计学方法并不是本书的重点，因此可以酌情跳到[下一节](#logical-operation)。
...
#### repeat循环 {#control-flow-rigorous-repeat}
...
它执行了三次计算：`seq(1, 5, length.out = 5)`, `seq(5, 25, length.out = 5)`和`seq(20, 100, length.out = 5)`.
...
### `purrr` package中的`apply`家族函数替代品和进化产物

#### `map`, `map_dbl`, `map_chr`, ...

#### `map2`和`pmap`

#### `invoke_map`, `invoke_map_dbl`, ...
...
使用`assign()`函数，在函数内赋值Global environment里的对象。
...
在这个例子中，`fun2()`赋值了`fun1()`里的`n`, 但`.GlobalEnv`里的`n`不受影响。当在“第一层”的函数里使用`<<-`, `.GlobalEnv`里对应的对象就会受到影响，即和`assign("x", value, envir = .GlobalEnv)`等效。
...
(@) **分割时间为时和分。**写名为`hour()`, `minute()`的函数，使得：
```{r include=FALSE}
...
(@) `seq(0, 20, 5)`, `seq(by = 5, 0, 20`, 和`seq(by = 5, 0, y = 30, 20)`的结果分别是什么？为什么？（你可能需要参考第\ref(abbr)节和第\ref(about-dot-dot-dot)节）

### 进阶 {#test-base-advanced}

(@) **质数表。**创建一个`prime.list()`函数，使`prime.list(i)`得到$(2, 3, 5, 7, 11, ... n)$, 其中`i`为大于或等于`3`的整数，$n$为小于`i`的最大质数。
