...
`lapply()`和`sapply()`本质上是把一个函数应用在**一个**向量/列表上，即这个向量/列表作为函数唯一的“自变量”。`Map()`则可以使用多组自变量。这意味着，`lapply()`能做到的，`Map`都能做到；`Map`能做到的，`lapply()`不一定做得到。
...
`<<-`可用于赋值“上一层”里的对象。当在“第一层”的函数里使用`<<-`时， `.GlobalEnv`里对应的对象就会受到影响，即和`assign("x", value, envir = .GlobalEnv)`等效。
